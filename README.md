...
- 员工可在「个人设置」中自主修改密码（需验证旧密码，新密码至少 8 位，修改后自动退出登录）
- 个人设置集中管理登录密码、锁屏密码和锁屏保护状态，采用设置项列表展示当前状态与可执行操作
...
- 侧边栏提供独立的「个人设置」入口，个人中心只保留资料概览和快捷操作
- 「登录安全」使用设置项列表布局，分别展示登录密码、锁屏密码和移除锁屏保护
- 登录密码显示修改说明并通过弹窗修改
...
- 上传同时校验扩展名、客户端 `Content-Type` 和文件头魔数；拒绝空文件以及扩展名与实际内容不匹配的文件
- 文件先写入临时 `.upload` 文件，再通过原子移动落盘；数据库事务失败时清理已写入文件
...
- 审批审核和薪资更新使用数据库条件更新实现并发保护：同一审批只能由一个请求成功处理，工资更新必须匹配读取时的当前值
...
- 文件上传已增加扩展名、`Content-Type` 和文件头魔数校验；生产环境仍建议接入病毒扫描和更深入的 Office/ZIP 内容检查
...
| 变量名 | 默认值 | 说明 |
| --- | --- | --- |
| `SERVER_ADDRESS` | `127.0.0.1` | 服务监听地址；测试环境默认仅本机访问，需要局域网访问时显式设置为 `0.0.0.0` |
