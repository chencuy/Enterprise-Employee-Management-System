(function () {
  ...
  const BULK_ATTENDANCE_PICKER_PAGE_SIZE = 3;
  ...
  const app = createApp({
      ...
      <div v-else class="app-shell">
        <aside class="sidebar">
          ...
          <div class="user-block">
            ...
            <span>{{ departmentDisplay(user) }}</span>
          </div>
          <nav class="nav-list">
        ...
        <main class="workspace">
          ...
          <section v-if="view === 'profile'" class="profile-grid">
            ...
            <div class="info-panel">
              ...
              <dl class="info-list">
                ...
                <div><dt>所属部门</dt><dd>{{ departmentDisplay(profile) }}</dd></div>
                <div><dt>对应主管</dt><dd>{{ nameWithId(profile?.managerName, profile?.managerId) }}</dd></div>
            ...
            <div class="message-panel">
              ...
              <div class="message-preview" v-if="unreadMessages.length">
                <button v-for="message in unreadMessages.slice(0, 3)" :key="message.id" @click="openMessage(message)">
                  <span>{{ nameWithId(message.senderName, message.senderId) }} 发来消息</span>
          ...
          <section v-if="view === 'employees'" class="work-section">
                ...
                <table class="list-table employee-table">
                  ...
                  <tbody>
                    <tr v-for="employee in employees.records" :key="employee.id">
                      <td>{{ nameWithId(employee.name, employee.id) }}</td>
                      <td>{{ employee.username }}</td>
                      <td>{{ departmentDisplay(employee, '-') }}</td>
            ...
            <div v-if="employeeFilterModalOpen" class="modal-backdrop" @click.self="closeEmployeeFilterModal">
              <form class="form-panel modal-panel wide-modal" @submit.prevent="searchEmployees">
                ...
                <div class="modal-grid">
                  ...
                  <label v-if="isAdmin"><span>部门</span><select v-model="employeeFilter.departmentId"><option value="">全部</option><option v-for="department in departments" :key="department.id" :value="department.id">{{ departmentDisplay(department) }}</option></select></label>
            ...
            <div v-if="employeeExportModalOpen" class="modal-backdrop" @click.self="closeEmployeeExportModal">
              <form class="form-panel modal-panel" @submit.prevent="exportEmployees">
                ...
                <label v-if="isAdmin"><span>部门</span><select v-model="employeeExportForm.departmentId"><option value="">全部</option><option v-for="department in departments" :key="department.id" :value="department.id">{{ departmentDisplay(department) }}</option></select></label>
            ...
            <div v-if="employeeModalOpen" class="modal-backdrop" @click.self="closeEmployeeModal">
              <form class="form-panel modal-panel" @submit.prevent="saveEmployee">
                ...
                <label v-if="isAdmin"><span>部门</span><select v-model="employeeForm.departmentId"><option value="">总部管理</option><option v-for="department in departments" :key="department.id" :value="department.id">{{ departmentDisplay(department) }}</option></select></label>
          ...
          <section v-if="view === 'departments'" class="work-section">
                ...
                <table class="list-table department-table">
                  ...
                  <tbody>
                    <tr v-for="department in departmentPage.records" :key="department.id">
                      <td>{{ departmentDisplay(department) }}</td>
                      <td>{{ nameWithId(department.managerName, department.managerId) }}</td>
            ...
            <div v-if="departmentModalOpen" class="modal-backdrop" @click.self="closeDepartmentModal">
              <form class="form-panel modal-panel" @submit.prevent="saveDepartment">
                ...
                <label><span>部门主管</span><select v-model="departmentForm.managerId"><option value="">暂不指定</option><option v-for="employee in supervisorOptions" :key="employee.id" :value="employee.id">{{ employeeDisplay(employee, '未分配') }}</option></select></label>
          ...
          <section v-if="view === 'salaries'" class="work-section">
            ...
            <div v-if="salaryModalOpen" class="modal-backdrop" @click.self="closeSalaryModal">
              <div class="form-panel modal-panel">
                ...
                <label><span>员工</span><select v-model="salaryForm.employeeId" @change="loadSalaryHistory"><option value="">请选择</option><option v-for="employee in employeeOptions" :key="employee.id" :value="employee.id">{{ employeeDisplay(employee) }}</option></select></label>
                ...
                <table class="list-table audit-table">
                  ...
                  <tbody>
                    <tr v-for="log in auditLogs.records" :key="log.id">
                      <td>{{ nameWithId(log.operatorName, log.operatorId) }}</td>
                      <td>{{ log.module }}</td>
                ...
                <table class="list-table leave-table">
                  ...
                  <tbody>
                    <tr v-for="item in leaves" :key="item.id">
                      <td>{{ nameWithId(item.employeeName, item.employeeId) }}</td>
                      <td>{{ departmentDisplay(item) }}</td>
                      ...
                      <td>{{ nameWithId(item.approverName, item.approverId) }}</td>
            ...
            <div v-if="leaveReviewModalOpen" class="modal-backdrop" @click.self="closeLeaveReview">
              <form class="form-panel modal-panel" @submit.prevent="submitLeaveReview">
                ...
                <div class="reply-context">
                  <span>{{ nameWithId(leaveReviewTarget?.employeeName, leaveReviewTarget?.employeeId) }}：{{ leaveReviewTarget?.startDate }} 至 {{ leaveReviewTarget?.endDate }}</span>
                </div>
          ...
          <section v-if="view === 'approvals'" class="work-section">
                ...
                <table class="list-table approval-table">
                  ...
                  <tbody>
                    <tr v-for="item in approvals" :key="item.id">
                      ...
                      <td>{{ nameWithId(item.applicantName, item.applicantId) }}</td>
                      <td>{{ departmentDisplay({ departmentId: item.applicantDepartmentId, departmentName: item.applicantDepartmentName }) }}</td>
                      ...
                      <td>{{ nameWithId(item.reviewerName, item.reviewerId) }}</td>
                      <td>{{ formatDateTime(item.createdAt) }}</td>
                      <td class="row-actions">
              ...
              <form class="form-panel modal-panel" @submit.prevent="submitApproval">
                ...
                <label v-if="approvalForm.type === 'TRANSFER'">
                  ...
                  <select v-model="approvalForm.targetDepartmentId" required>
                    ...
                    <option v-for="department in departments" :key="department.id" :value="department.id">{{ departmentDisplay(department) }}</option>
                  </select>
                </label>
            ...
            <div v-if="approvalReviewModalOpen" class="modal-backdrop" @click.self="closeApprovalReview">
              <form class="form-panel modal-panel" @submit.prevent="submitApprovalReview">
                ...
                <div class="reply-context">
                  <span>{{ nameWithId(approvalReviewTarget?.applicantName, approvalReviewTarget?.applicantId) }}，{{ approvalTypeLabel(approvalReviewTarget?.type) }}，{{ approvalDetail(approvalReviewTarget) }}</span>
                </div>
          ...
          <section v-if="view === 'messages'" class="work-section">
                ...
                <table class="list-table message-table">
                  ...
                  <tbody>
                    <tr v-for="message in messages" :key="message.id" :class="['message-row', { unread: !message.readFlag && message.receiverId === user.id, expanded: expandedMessageId === message.id }]" @click="toggleMessage(message)">
                      <td>{{ nameWithId(message.senderName, message.senderId) }}</td>
                      <td>{{ nameWithId(message.receiverName, message.receiverId) }}</td>
            ...
            <div v-if="messageModalOpen" class="modal-backdrop" @click.self="closeMessageModal">
              <form class="form-panel modal-panel" @submit.prevent="sendMessage">
                ...
                <label v-else><span>收件人</span><select v-model="messageForm.receiverId"><option value="">请选择</option><option v-for="contact in contacts" :key="contact.id" :value="contact.id">{{ personRoleDisplay(contact) }}</option></select></label>
                  ...
                  <div class="file-target-col">
                    ...
                    <div class="file-target-group">
                      <label v-for="dept in departments" :key="'d' + dept.id" class="file-target-item">
                        ...
                        <span>{{ departmentDisplay(dept) }}</span>
                      </label>
                  ...
                  <div class="file-target-col">
                    ...
                    <div class="file-target-group">
                      <label v-for="person in employeeOptions" :key="'e' + person.id" class="file-target-item">
                        ...
                        <span>{{ personRoleDisplay(person) }}</span>
                      </label>
                  ...
                  <table class="list-table file-detail-table">
                    ...
                    <tbody>
                      <tr v-for="detail in fileRecipientDetails" :key="detail.employeeId">
                        <td>{{ nameWithId(detail.employeeName, detail.employeeId) }}</td>
                        <td>{{ departmentDisplay(detail) }}</td>
          ...
          <section v-if="view === 'attendance'" class="work-section">
            <div class="table-panel">
                ...
                <table class="list-table attendance-table">
                  ...
                  <tbody>
                    <tr v-for="record in attendances" :key="record.id">
                      <td>{{ nameWithId(record.employeeName || user.name, record.employeeId || user.id) }}</td>
                      <td>{{ departmentDisplay(record) }}</td>
              ...
              <div class="pagination inline-actions">
                <div class="pagination-actions">
                  <button v-if="!isAdmin" class="primary-button" :disabled="attendanceUnreadCount === 0" @click="checkInAttendance"><i data-lucide="check-circle"></i><span>{{ checkInButtonText }}</span></button>
            ...
            <div v-if="attendanceFilterModalOpen" class="modal-backdrop" @click.self="closeAttendanceFilterModal">
              <form class="form-panel modal-panel" @submit.prevent="searchAttendance">
                ...
                <label v-if="isAdmin || isSupervisor"><span>员工</span><select v-model="attendanceFilter.employeeId"><option value="">{{ isAdmin ? '全部' : '本人' }}</option><option v-for="employee in attendanceEmployeeOptions" :key="employee.id" :value="employee.id">{{ employeeDisplay(employee) }}</option></select></label>
            ...
            <div v-if="attendanceExportModalOpen" class="modal-backdrop" @click.self="closeAttendanceExportModal">
              <form class="form-panel modal-panel" @submit.prevent="exportAttendance">
                ...
                <label v-if="isAdmin || isSupervisor"><span>员工</span><select v-model="attendanceExportForm.employeeId"><option value="">{{ isAdmin ? '全部' : '本人' }}</option><option v-for="employee in attendanceEmployeeOptions" :key="employee.id" :value="employee.id">{{ employeeDisplay(employee) }}</option></select></label>
            ...
            <div v-if="attendanceSettingsModalOpen" class="modal-backdrop" @click.self="closeAttendanceSettingsModal">
              <div class="form-panel modal-panel wide-modal attendance-settings-modal">
                ...
                <form class="attendance-settings-section" @submit.prevent="forceAbsentAttendance">
                  ...
                  <div class="attendance-time-grid">
                    <label><span>员工</span><select v-model="attendanceForceAbsentForm.employeeId" required><option value="">请选择</option><option v-for="employee in attendanceActionEmployeeOptions" :key="'absent-' + employee.id" :value="employee.id">{{ employeeDisplay(employee) }}</option></select></label>
            ...
            <div v-if="bulkAttendanceModalOpen" class="modal-backdrop" @click.self="closeBulkAttendanceModal">
              <form class="form-panel modal-panel wide-modal" @submit.prevent="bulkFullAttendance">
                ...
                <div class="file-target-block bulk-attendance-picker">
                  <div class="file-target-col">
                    <div class="file-target-title">
                      <strong>按部门选择</strong>
                      <span>部门内在职人员</span>
                    </div>
                    <label class="file-target-search">
                      <input
                        v-model.trim="bulkAttendanceDepartmentSearch"
                        type="search"
                        placeholder="搜索部门ID或名称"
                        autocomplete="off"
                        @input="bulkAttendanceDepartmentPage = 1"
                      >
                    </label>
                    <div class="file-target-group bulk-department-group">
                      <label v-for="department in pagedBulkAttendanceDepartments" :key="'bulk-d' + department.id" class="file-target-item">
                        <input type="checkbox" :value="department.id" v-model="bulkAttendanceForm.departmentIds">
                        <span>{{ departmentDisplay(department) }}</span>
                      </label>
                      <p v-if="!bulkAttendanceDepartmentOptions.length" class="empty-text">暂无部门</p>
                    </div>
                    <div class="file-target-pagination">
                      <button type="button" class="ghost-button compact-button" :disabled="activeBulkAttendanceDepartmentPage <= 1" @click="bulkAttendanceDepartmentPageTo(activeBulkAttendanceDepartmentPage - 1)">上一页</button>
                      <span>共 {{ bulkAttendanceDepartmentOptions.length }} 个部门 · 第 {{ activeBulkAttendanceDepartmentPage }} / {{ bulkAttendanceDepartmentTotalPages }} 页</span>
                      <button type="button" class="ghost-button compact-button" :disabled="activeBulkAttendanceDepartmentPage >= bulkAttendanceDepartmentTotalPages" @click="bulkAttendanceDepartmentPageTo(activeBulkAttendanceDepartmentPage + 1)">下一页</button>
                    </div>
                  </div>
                  <div class="file-target-col">
                    <div class="file-target-title">
                    ...
                    <label class="file-target-search">
                      <input
                        v-model.trim="bulkAttendanceEmployeeSearch"
                        type="search"
                        placeholder="搜索员工ID、姓名、账号或部门"
                        autocomplete="off"
                        @input="bulkAttendanceEmployeePage = 1"
                      >
                    </label>
                    <div class="file-target-group bulk-employee-group">
                      <label v-for="employee in pagedBulkAttendanceEmployees" :key="'bulk-e' + employee.id" :class="['file-target-item', { disabled: !canBulkAttendanceEmployee(employee) }]">
                        <input type="checkbox" :value="employee.id" v-model="bulkAttendanceForm.employeeIds" :disabled="!canBulkAttendanceEmployee(employee)">
                        <span>{{ employeeDisplay(employee) }}</span>
                        <small v-if="!canBulkAttendanceEmployee(employee)">{{ statusText(employee) }}</small>
                      </label>
                      <p v-if="!bulkAttendanceEmployeeSearchOptions.length" class="empty-text">暂无人员</p>
                    </div>
                    <div class="file-target-pagination">
                      <button type="button" class="ghost-button compact-button" :disabled="activeBulkAttendanceEmployeePage <= 1" @click="bulkAttendanceEmployeePageTo(activeBulkAttendanceEmployeePage - 1)">上一页</button>
                      <span>共 {{ bulkAttendanceEmployeeSearchOptions.length }} 人 · 第 {{ activeBulkAttendanceEmployeePage }} / {{ bulkAttendanceEmployeeTotalPages }} 页</span>
                      <button type="button" class="ghost-button compact-button" :disabled="activeBulkAttendanceEmployeePage >= bulkAttendanceEmployeeTotalPages" @click="bulkAttendanceEmployeePageTo(activeBulkAttendanceEmployeePage + 1)">下一页</button>
                    </div>
                  </div>
                </div>
          ...
          <section v-if="view === 'attendanceAnomalies'" class="work-section">
            ...
            <div v-if="attendanceAnomalyFilterModalOpen" class="modal-backdrop" @click.self="closeAttendanceAnomalyFilterModal">
              <form class="form-panel modal-panel" @submit.prevent="searchAttendanceAnomalies">
                ...
                <label v-if="isAdmin"><span>部门</span><select v-model="attendanceAnomalyFilter.departmentId"><option value="">全部</option><option v-for="department in departments" :key="department.id" :value="department.id">{{ departmentDisplay(department) }}</option></select></label>
                <label><span>员工</span><select v-model="attendanceAnomalyFilter.employeeId"><option value="">全部</option><option v-for="employee in attendanceEmployeeOptions" :key="employee.id" :value="employee.id">{{ employeeDisplay(employee) }}</option></select></label>
                ...
                <table class="list-table login-log-table">
                  ...
                  <tbody>
                    <tr v-for="log in loginLogs.records" :key="log.id">
                      <td>{{ log.username || '-' }}</td>
                      <td>{{ nameWithId(log.employeeName, log.employeeId) }}</td>
                      <td>{{ log.ipAddress || '-' }}</td>
            ...
            <div class="announcement-list">
              <article v-for="announcement in announcements" :key="announcement.id" :class="['announcement-item', { pinned: announcement.isPinned }]">
                <header>
                  ...
                  <span>{{ nameWithId(announcement.publisherName || '管理员', announcement.publisherId) }} · {{ formatDateTime(announcement.createdAt) }}</span>
                  </div>
            ...
            <article class="announcement-item required">
              <header>
                <div>
                  ...
                  <span>{{ nameWithId(currentPendingAnnouncement.publisherName || '管理员', currentPendingAnnouncement.publisherId) }} · {{ formatDateTime(currentPendingAnnouncement.createdAt) }}</span>
                </div>
              </header>
    ...
    data() {
      return {
        ...
        bulkAttendanceDepartmentPage: 1,
        bulkAttendanceDepartmentSearch: '',
        bulkAttendanceEmployeePage: 1,
        bulkAttendanceEmployeeSearch: '',
    ...
    computed: {
      ...
      replyTargetText() {
        ...
        return this.nameWithId(this.messageForm.receiverName, this.messageForm.receiverId) + '（' + this.roleLabel(this.messageForm.receiverRole) + '）';
      },
      supervisorOptions() {
      ...
      attendanceEmployeeOptions() {
        if (this.isAdmin) {
          return this.attendanceEmployeeChoices.filter(employee => employee.role !== 'ADMIN');
        }
        if (this.isSupervisor) {
          return this.attendanceEmployeeChoices.filter(employee => employee.role !== 'ADMIN');
        }
      ...
      attendanceActionEmployeeOptions() {
        return this.attendanceEmployeeChoices.filter(employee => employee.role !== 'ADMIN' && employee.status === 'WORKING');
      },
      bulkAttendanceDepartmentOptions() {
        const keyword = String(this.bulkAttendanceDepartmentSearch || '').trim().toLowerCase();
        if (!keyword) {
        ...
        return this.departments.filter(department => {
          const values = [
            department.id,
            department.name,
            this.departmentDisplay(department)
          ];
          return values.some(value => String(value || '').toLowerCase().includes(keyword));
        });
      },
      bulkAttendanceDepartmentTotalPages() {
        return Math.max(1, Math.ceil(this.bulkAttendanceDepartmentOptions.length / BULK_ATTENDANCE_PICKER_PAGE_SIZE));
      },
      activeBulkAttendanceDepartmentPage() {
        return Math.min(Math.max(Number(this.bulkAttendanceDepartmentPage) || 1, 1), this.bulkAttendanceDepartmentTotalPages);
      },
      pagedBulkAttendanceDepartments() {
        const start = (this.activeBulkAttendanceDepartmentPage - 1) * BULK_ATTENDANCE_PICKER_PAGE_SIZE;
        return this.bulkAttendanceDepartmentOptions.slice(start, start + BULK_ATTENDANCE_PICKER_PAGE_SIZE);
      },
      bulkAttendanceEmployeeSearchOptions() {
        const keyword = String(this.bulkAttendanceEmployeeSearch || '').trim().toLowerCase();
        const options = this.attendanceEmployeeChoices.filter(employee => employee.role !== 'ADMIN');
        if (!keyword) {
          return options;
        }
        return options.filter(employee => {
          const values = [
            ...
            this.employeeDisplay(employee)
          ];
          return values.some(value => String(value || '').toLowerCase().includes(keyword));
        });
      },
      bulkAttendanceEmployeeTotalPages() {
        return Math.max(1, Math.ceil(this.bulkAttendanceEmployeeSearchOptions.length / BULK_ATTENDANCE_PICKER_PAGE_SIZE));
      },
      activeBulkAttendanceEmployeePage() {
        return Math.min(Math.max(Number(this.bulkAttendanceEmployeePage) || 1, 1), this.bulkAttendanceEmployeeTotalPages);
      },
      pagedBulkAttendanceEmployees() {
        const start = (this.activeBulkAttendanceEmployeePage - 1) * BULK_ATTENDANCE_PICKER_PAGE_SIZE;
        return this.bulkAttendanceEmployeeSearchOptions.slice(start, start + BULK_ATTENDANCE_PICKER_PAGE_SIZE);
      },
      ...
      checkInButtonText() {
        if (this.user?.status && this.user.status !== 'WORKING') {
          return '无需签到';
        }
        return this.attendanceUnreadCount === 0 ? '今日已签到' : '今日签到';
      }
    },
    ...
    methods: {
      ...
      async loadBaseData() {
        await Promise.all([
          ...
          this.loadAttendanceEmployeeChoices(),
      ...
      async refreshCurrent() {
        ...
        if (this.view === 'attendance') {
          await Promise.all([this.loadAttendance(), this.loadAttendanceUnread(), this.loadCheckInWindow(), this.loadAttendanceEmployeeChoices()]);
        }
        if (this.view === 'attendanceAnomalies') {
          await Promise.all([this.loadAttendanceAnomalies(), this.loadDepartments(), this.loadAttendanceEmployeeChoices()]);
        }
      },
      async loadProfile() {
      ...
      async loadAttendanceEmployeeChoices() {
        this.attendanceEmployeeChoices = await this.api('/api/employees/attendance-options');
      },
      async loadEmployees() {
      ...
      openBulkAttendanceModal() {
        ...
        this.bulkAttendanceDepartmentPage = 1;
        this.bulkAttendanceDepartmentSearch = '';
        this.bulkAttendanceEmployeePage = 1;
        this.bulkAttendanceEmployeeSearch = '';
      ...
      bulkAttendanceDepartmentPageTo(page) {
        this.bulkAttendanceDepartmentPage = Math.min(Math.max(Number(page) || 1, 1), this.bulkAttendanceDepartmentTotalPages);
      },
      bulkAttendanceEmployeePageTo(page) {
        this.bulkAttendanceEmployeePage = Math.min(Math.max(Number(page) || 1, 1), this.bulkAttendanceEmployeeTotalPages);
      },
      bulkAttendanceEmployeeSearchTo(keyword) {
        this.bulkAttendanceEmployeeSearch = keyword;
        this.bulkAttendanceEmployeePage = 1;
      },
      ...
      async saveEmployee() {
        ...
        try {
          ...
          await Promise.all([this.loadEmployees(), this.loadEmployeeOptions(), this.loadAttendanceEmployeeChoices(), this.loadDepartments()]);
        } catch (error) {
          this.showFeedback('error', error.message);
        }
      },
      async removeEmployee(employee) {
        if (!window.confirm('确认删除员工 ' + this.nameWithId(employee.name, employee.id) + '？')) {
          return;
        }
        try {
      ...
      async adminClearLockPassword(employee) {
        if (!window.confirm('确认强制解除 ' + this.nameWithId(employee.name, employee.id) + ' 的锁屏密码？')) return;
        try {
          ...
          this.showFeedback('success', '已解除 ' + this.nameWithId(employee.name, employee.id) + ' 的锁屏密码');
          await this.loadEmployees();
        } catch (error) {
          this.showFeedback('error', error.message);
        }
      ...
      async salaryAction(url, requireAmount) {
        ...
        if (url.endsWith('/decrease')) {
          const employeeName = this.selectedSalaryEmployee ? this.nameWithId(this.selectedSalaryEmployee.name, this.selectedSalaryEmployee.id) : '该员工';
      ...
      approvalDetail(item) {
        ...
        if (item.type === 'TRANSFER') {
          return '目标部门：' + this.departmentDisplay({
      ...
      departmentDisplay(department, fallback = '总部管理') {
        if (!department) {
          return fallback;
        }
        const id = department.id ?? department.departmentId;
        const name = department.name ?? department.departmentName;
        return this.nameWithId(name, id, fallback);
      },
      employeeDisplay(employee, departmentFallback = '总部管理') {
        if (!employee) {
          return '-';
        }
        const base = this.nameWithId(employee.name, employee.id);
        const department = this.departmentDisplay(employee, departmentFallback);
        return base + '（' + department + '）';
      },
      personRoleDisplay(person) {
        if (!person) {
          return '-';
        }
        return this.nameWithId(person.name, person.id) + '（' + this.roleLabel(person.role) + '）';
      },
      canBulkAttendanceEmployee(employee) {
        return employee && employee.role !== 'ADMIN' && employee.status === 'WORKING';
      },
      roleLabel(role) {
